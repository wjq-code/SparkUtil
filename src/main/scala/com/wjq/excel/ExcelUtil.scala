...
object ExcelUtil {
  ...
  def getExcelUtil(sparkSession: SparkSession, path: String) = {
    ...
    val order1 = rdd.filter(row => {
      val date2011 = row.getString(9)
      if (date2011 == "1.yes" || date2011 == "0.no") {
        true
      } else {
        false
      }
    ...
    order.map(row => {
      ...
      for (elem <- row._2) {
        sb =  new StringBuffer()
        addr = elem.getString(0)
        val x = elem.getString(1)
        val y = elem.getString(2)
        try {
...
//        })
      ...
      sb.append("2011").append("\t").append(count)
//      println(sb.toString)
      (row._1, sb)
    }).join(order1).map(row => {
      val monthCount = row._2._1.toString.split("\t")(4)
      println("monthCount" + monthCount)
      row._2._1.toString + "\t" + row._2._2 + "\t" + (monthCount.toFloat / row._2._2.toFloat)
    }).repartition(1).saveAsTextFile("file:///D:\\idea_workspace\\SparkUtil\\File\\2011")
  ...
  def main(args: Array[String]): Unit = {
    ...
    val path = "file:///D:\\idea_workspace\\SparkUtil\\File\\data1.xlsx"
